...
if (($package -join "`n") -notmatch 'versionCode=48\b' -or
    ($package -join "`n") -notmatch 'versionName=0\.21\.11\b') {
    throw "Device rebooted, but ZuiControl is not 48/0.21.11. Log: $log"
}
Write-Host "9008 flash and reboot passed; device reports ZuiControl 48/0.21.11. Log: $log"
