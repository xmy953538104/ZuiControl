...
if (($package -join "`n") -notmatch 'versionCode=35\b' -or
    ($package -join "`n") -notmatch 'versionName=0\.20\.6\b') {
    throw "Device rebooted, but ZuiControl is not 35/0.20.6. Log: $log"
}
Write-Host "9008 flash and reboot passed; device reports ZuiControl 35/0.20.6. Log: $log"
