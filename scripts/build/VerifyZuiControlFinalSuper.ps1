...
if (-not $WorkDir) { $WorkDir = Join-Path $WorkspaceRoot 'zui072（flash）\work\temp\verify_final_super' }
...
try {
    ...
    $activityTaskSupervisorSmali = Require-SingleFile $services 'ActivityTaskSupervisor.smali'
    ...
    $markers = @(
        ...
        @{ Path = $serviceSmali; Needle = 'mLatestWindowFocusEmpty:Z' },
        @{ Path = $serviceSmali; Needle = '\nemptyFocusPolicy=retainLastNonEmptyWindow' },
        @{ Path = $serviceSmali; Needle = '\nemptyFocusTransitionCount=' },
        @{ Path = $serviceSmali; Needle = 'com.lenovo.screensplit' },
        @{ Path = $serviceSmali; Needle = 'com.zui.freeform.sidebar' },
        ...
        @{ Path = $serviceSmali; Needle = 'setModuleEnabled' },
        @{ Path = $serviceSmali; Needle = 'unsupported_module' },
        @{ Path = $serviceSmali; Needle = 'refresh_property_set' },
    ...
    $topResumedMethod = [regex]::Match(
        $topResumedText,
        '(?ms)^\.method public updateTopResumedActivityIfNeeded\(Ljava/lang/String;\)V\r?\n.*?^\.end method\r?$'
    )
    if (-not $topResumedMethod.Success) {
        throw 'Missing ActivityTaskSupervisor.updateTopResumedActivityIfNeeded in final services.jar.'
    }
    $assignment = 'iput-object v1, p0, Lcom/android/server/wm/ActivityTaskSupervisor;->mTopResumedActivity:Lcom/android/server/wm/ActivityRecord;'
    if ($topResumedMethod.Value.IndexOf($assignment, [StringComparison]::Ordinal) -lt 0 -or
    ...
    if ($ok -and -not $KeepWork) {
        ...
        Write-Warning "Final-super verification workspace kept for debugging: $WorkDir"
    }
}
