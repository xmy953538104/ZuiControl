...
$ExpectedVersionCode = '71'
$ExpectedVersionName = '0.21.34'
...
try {
    ...
    $AppApk = Join-Path $System 'priv-app\ZuiControlV71\ZuiControl.apk'
    $packageDirs = @(Get-ChildItem -LiteralPath (Join-Path $System 'priv-app') -Directory |
        Where-Object { $_.Name -match '^ZuiControlV\d+$' })
    if ($packageDirs.Count -ne 1 -or $packageDirs[0].Name -ne 'ZuiControlV71') {
        throw 'Expected exactly one ZuiControlV71 system package directory.'
    }
