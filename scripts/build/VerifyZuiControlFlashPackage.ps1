...
try {
    ...
    $AppApk = Join-Path $System 'priv-app\ZuiControlV63\ZuiControl.apk'
    $packageDirs = @(Get-ChildItem -LiteralPath (Join-Path $System 'priv-app') -Directory |
        Where-Object { $_.Name -match '^ZuiControlV\d+$' })
    if ($packageDirs.Count -ne 1 -or $packageDirs[0].Name -ne 'ZuiControlV63') {
        throw 'Expected exactly one ZuiControlV63 system package directory.'
    }
