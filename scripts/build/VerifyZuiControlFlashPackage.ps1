...
$ExpectedVersionCode = '64'
$ExpectedVersionName = '0.21.27'
...
try {
    ...
    $AppApk = Join-Path $System 'priv-app\ZuiControlV64\ZuiControl.apk'
    $packageDirs = @(Get-ChildItem -LiteralPath (Join-Path $System 'priv-app') -Directory |
        Where-Object { $_.Name -match '^ZuiControlV\d+$' })
    if ($packageDirs.Count -ne 1 -or $packageDirs[0].Name -ne 'ZuiControlV64') {
        throw 'Expected exactly one ZuiControlV64 system package directory.'
    }
    ...
    foreach ($healthKey in @('zui_control_uperf_health',
    ...
    Invoke-Checked $Python (Join-Path $RepoRoot 'tests\selinux\TestUperfRuntimeAccess.py') `
        --mode final `
        --system-root $System `
