...
$ExpectedVersionCode = '68'
$ExpectedVersionName = '0.21.31'
...
try {
    ...
    $AppApk = Join-Path $System 'priv-app\ZuiControlV68\ZuiControl.apk'
    $packageDirs = @(Get-ChildItem -LiteralPath (Join-Path $System 'priv-app') -Directory |
        Where-Object { $_.Name -match '^ZuiControlV\d+$' })
    if ($packageDirs.Count -ne 1 -or $packageDirs[0].Name -ne 'ZuiControlV68') {
        throw 'Expected exactly one ZuiControlV68 system package directory.'
    }
