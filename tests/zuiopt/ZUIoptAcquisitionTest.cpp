...
void initialize(){
    ...
    for(auto g:{"","/top-app","/background","/ZUIopt"})for(auto f:{"tasks","mems","cpus"})put(root+"/dev/cpuset"+g+"/"+f,f==std::string("mems")?"0":"0-7");
...
struct AcquisitionFixture:RuntimeFixture {
    ...
    BaselineResult acquire(ProcessState& p){return owner->acquire(p);}
    ...
    void place(){owner->prepare(p());for(auto& [tid,t]:p().tasks)owner->apply(p(),tid,t,cpus("2-6"),"default");}
...
void matrix(){
    ...
    for(int source=0;source<3;source++)for(int visible:{0,100,250,500,750}){
        ...
        Kernel::tasks.at(43).mask=visible?255:67;Kernel::tasks.at(43).group=visible?"/top-app":"/background";
        f.start(source);
        if(visible)f.noOwnership();
        ...
        for(int t:{100,250,500,750}){
            ...
            f.tick(t);if(t<visible)f.noOwnership();else require(f.p().managed&&!f.p().acquiring,"stable retry acquisition miss");
        }
        if(source==2)require(f.snapshotQueries==queries,"ProcessObserver-only retries queried activity");
        require(f.journal->commits==f.initialCommits+1&&f.journal->leases.size()==1,"duplicate sources acquired twice");
        f.place();f.background();f.empty();
    }
    ...
    for(bool custom:{false,true}){
        ...
        f.app.state=2;f.app.flags=4;f.primary();f.tick(3000);require(f.p().managed,"fresh authority did not retry");f.background();
    }
    ...
    for(bool reuse:{false,true}){
        ...
        require(f.p().managed,"exit/reuse observation recovery");f.background();
    }
    ...
    {AcquisitionFixture f;Kernel::hook=[](const std::string& path){if(path=="affinity/43"){Kernel::hook={};Kernel::tasks[44]={10000,"/top-app",255};}};
        f.start(2);f.noOwnership();f.tick(100);require(f.p().managed&&f.journal->entries.count(44)&&f.p().ownershipFloor>10000,"pending-born child baseline/floor");
    ...
    for(int mode=0;mode<5;mode++){
        ...
        if(mode==1){f.generation=20;Kernel::tasks.at(43).mask=255;f.primary();f.tick(100);require(f.p().managed&&f.journal->leases.at(42).processStart==20,"stale PID acquired");f.background();}
    }
...
void stress(){
    ...
    for(int i=0;i<512;i++){
        ...
        require(f.p().ownershipFloor>=static_cast<uint64_t>((100.+(base+visible)/1000.)*sysconf(_SC_CLK_TCK)),"lease predates uniform baseline");
        f.place();f.background();f.empty();
    }
