"""Changed-path monitor checks, no device manipulation or production fixture mutation."""
...
with tempfile.TemporaryDirectory(prefix='zuicontrol-monitor-') as out:
    subprocess.run(['javac', '-d', out, str(src/'MonitorSnapshot.java'),
                    str(src/'MonitorLifecycle.java'), str(Path(__file__).with_name('MonitorTest.java'))], check=True)
    subprocess.run(['java', '-cp', out, 'com.zui.server.control.MonitorTest'], check=True)
collector = (src/'MonitorCollector.java').read_text(encoding='utf-8')
overlay = (root/'app/src/main/java/com/zui/zuicontrol/PerformanceMonitor.kt').read_text(encoding='utf-8')
for forbidden in ('ProcessBuilder', 'Runtime.getRuntime', 'FileOutputStream', 'setAffinity', 'renice', 'setPriority'):
    assert forbidden not in collector
assert 'removeCallbacksAndMessages(null)' in collector and 'quitSafely()' in collector
assert 'ticket == epoch && handler != null' in collector
assert collector.count('postDelayed(') == 1
assert 'FLAG_NOT_FOCUSABLE' in overlay and 'FLAG_NOT_TOUCHABLE' in overlay
assert '/proc/' not in overlay and 'postDelayed' not in overlay
assert 'IBinder.FLAG_ONEWAY' in collector and 'linkToDeath' in collector
assert 'getCallingUid() != 1000' in overlay
service = (src/'ZuiControlService.java').read_text(encoding='utf-8')
assert '!mRawFocusTransient && pkg.equals(mRawFocusedPackage) && user == mRawFocusedUserId' in service
assert 'mRawFocusTransient = transientFocus;\n        refreshMonitor();' in service
print('MONITOR_STATIC_SINGLE_PIPELINE_AND_READONLY=PASS (device lifecycle proof still required)')

# Execute the production collector lifecycle with a deterministic Android looper fixture.
# Proc math is covered above; these stubs do not claim real-device permissions or overhead.
stubs = {
...
with tempfile.TemporaryDirectory(prefix='zuicontrol-monitor-lifecycle-') as directory:
    ...
    subprocess.run(['javac', '-d', directory, *files, str(src/'MonitorCollector.java'), str(src/'MonitorSnapshot.java')], check=True)
    subprocess.run(['java', '-cp', directory, 'com.zui.server.control.CollectorTest'], check=True)
