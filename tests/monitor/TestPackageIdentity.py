...
class PackageIdentity(unittest.TestCase):
    def test_identity_bindings(self):
        self.assertEqual(APP_APK_PATH,'system/priv-app/ZuiControlV61/ZuiControl.apk')
        gradle=(ROOT/'app/build.gradle.kts').read_text(encoding='utf8')
        self.assertIn('versionCode = 61',gradle)
        self.assertIn('versionName = "0.21.24"',gradle)
        for name in ('.github/workflows/build.yml','scripts/build/BuildZuiControl.ps1',
            ...
            self.assertIn('ZuiControlV61',(ROOT/name).read_text(encoding='utf8'))
