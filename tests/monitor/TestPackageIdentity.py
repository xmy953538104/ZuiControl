...
class PackageIdentity(unittest.TestCase):
    def test_identity_bindings(self):
        self.assertEqual(APP_APK_PATH,'system/priv-app/ZuiControlV63/ZuiControl.apk')
        gradle=(ROOT/'app/build.gradle.kts').read_text(encoding='utf8')
        self.assertIn('versionCode = 63',gradle)
        self.assertIn('versionName = "0.21.26"',gradle)
        for name in ('.github/workflows/build.yml','scripts/build/BuildZuiControl.ps1',
            ...
            self.assertIn('ZuiControlV63',(ROOT/name).read_text(encoding='utf8'))
