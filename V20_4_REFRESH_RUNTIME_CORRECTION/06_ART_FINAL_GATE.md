...
9. 版本化fixed-seven package Preflight PASS：rawprogram精确7项，1个rawprogram、0个patch XML，四镜像hash与candidate一致；随后source verifier再次返回 `ok=true`。Preflight模式在ADB/EDL代码前返回，设备未重启、未刷写。

最小证据：[`final_super_receipt`](raw/final_super_receipt_20260831170720.txt)、[`ART gate`](raw/final_art_gate_20260831170720/)、[`policy gate`](raw/final_policy_gate_20260831170720/)、[`init exact-file gate`](raw/final_init_gate_20260831170720/)、[`build provenance`](raw/build_20260831170720/)、[`fixed-seven Preflight`](raw/fixed_seven_preflight_20260831170720/)。
