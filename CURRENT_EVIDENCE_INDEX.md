...
## V20.4 Runtime Correction 当前证据

设备仍运行旧fixed RunId `20260831134511`；下列是定向correction的probe/source/host证据，不是刷后PASS：

- `KILL_SWITCH_ROOT_CAUSE_CONFIRMED=RAW_SETPROP_DOES_NOT_REPORT_PROCESS_SYSPROP_CHANGE`。raw setprop后末样本`2127.214ms`仍mask0；标准 `SYSPROPS_TRANSACTION` 后首样本`59.772ms`为mask2并释放ownership；恢复poke首样本`56.350ms`重建。最小报告：[`01_KILL_SWITCH_ROOT_CAUSE.md`](V20_4_REFRESH_RUNTIME_CORRECTION/01_KILL_SWITCH_ROOT_CAUSE.md)。最小raw：[`02_after_setprop_before_poke.csv`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/02_after_setprop_before_poke.csv)、[`03_after_poke.csv`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/03_after_poke.csv)、[`05_restore_before_poke.csv`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/05_restore_before_poke.csv)、[`06_restore_after_poke.csv`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/06_restore_after_poke.csv)、[`08_command_transcript.txt`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/08_command_transcript.txt)、[`09_final_restored_baseline.txt`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/sysprop_poke_20260831/09_final_restored_baseline.txt)。
- correction source：`146e096c6a6bc8b3fee60349b856990fd9fb68d2`。TX10为reserved signed-App strict-auth API，直接persist+transition；当前UI未接线。raw engineering property由init edge-only、经 `shell_exec` entrypoint 的标准poke唤醒，无polling/常驻notifier。设计：[`02_KILL_SWITCH_EVENT_DESIGN.md`](V20_4_REFRESH_RUNTIME_CORRECTION/02_KILL_SWITCH_EVENT_DESIGN.md)。
- null-gap：空Window不再创建default120，也不覆盖最后非空snapshot；100轮host模型为200真实owner apply、0个default120 intermediate。报告：[`03_NULL_WINDOW_TRANSITION.md`](V20_4_REFRESH_RUNTIME_CORRECTION/03_NULL_WINDOW_TRANSITION.md)。
- OEM exact registry：仅新增 `com.lenovo.screensplit` 与 `com.zui.freeform.sidebar`；不泛化 `com.zui.*` / system App。报告：[`04_OEM_TRANSIENT_REGISTRY.md`](V20_4_REFRESH_RUNTIME_CORRECTION/04_OEM_TRANSIENT_REGISTRY.md)。
- `RUNTIME_CORRECTION_HOST=39/39_PASS`，V20.3B regression=`5/5_PASS`，Java8/D8 PASS。证据：[`05_HOST_TESTS.md`](V20_4_REFRESH_RUNTIME_CORRECTION/05_HOST_TESTS.md)。
- 未刷入候选 RunId `20260831170720`：source `146e096...` / CI `33375509612` / final-super `dc4fd4bc...c269d` / marker 56 / final `services.jar` `0b7bb46c...`；ART `DEX_RC=0/GATE_RC=0`，split CIL `SECILC_RC=0/GATE_RC=0`，官方host init exact-file `PASS`，版本化fixed-seven package Preflight `PASS`（精确7项、0 patch XML、source verifier `ok=true`）。`PRE_FLASH_READY=YES / FLASHED=NO / BOOT_HARD_GATE=PENDING_POST_FLASH / DEVICE_VALIDATION=PENDING / APP_UI_TX10=NOT_EXECUTED`。Gate：[`06_ART_FINAL_GATE.md`](V20_4_REFRESH_RUNTIME_CORRECTION/06_ART_FINAL_GATE.md)；compact build：[`build_20260831170720/`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/build_20260831170720/)；final-super receipt：[`final_super_receipt_20260831170720.txt`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/final_super_receipt_20260831170720.txt)；fixed-seven receipt：[`fixed_seven_preflight_20260831170720/`](V20_4_REFRESH_RUNTIME_CORRECTION/raw/fixed_seven_preflight_20260831170720/)。
